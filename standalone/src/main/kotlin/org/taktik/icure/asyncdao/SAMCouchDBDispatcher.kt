...
class SAMCouchDBDispatcher(
    ...
    private val connectors =
        AsyncSafeCache<CouchDbConnectorReference, Client>(
            CaffeineCache(
                "Connectors",
                Caffeine
                    .newBuilder()
        ...
        try {
            connectors.get(
                ...
                ?: if (trials >
                    ...
                    attemptConnection(datastoreInformation, trials - 1)
                } else {
                    throw IllegalStateException("Cannot instantiate client")
                }
        } catch (e: Exception) {
    ...
    override suspend fun getClient(
        datastoreInformation: IDatastoreInformation,
        retry: Int,
    ): Client = attemptConnection(datastoreInformation, retry)

    private data class CouchDbConnectorReference(
        val datastoreInformation: IDatastoreInformation,
    )
}
