...
class SAMCouchDBCredentialsProvider(
    ...
    @Volatile
    private var currentCredentials =
        UsernamePassword(
            ...
            runBlocking {
                val initialCredentialsOk =
                    checkCredentialsSameOnAllClusters(it)?.also { status ->
    ...
    override suspend fun notifyNewCredentials(notification: RotationNotification): RotationResponse =
        properties.knownServerUrls().let { knownServers ->
            if (!notification.affectedServices.containsAll(knownServers)) {
                log.warn(
                    "Received credentials update notification which does not include all known couchdb servers (${notification.affectedServices}",
                )
        ...
        properties.knownServerUrls().asFlow().fold(null as String?) { err, currUrl ->
            err ?: try {
                if (checkCredentialsSameOnAllNodes(credentials, currUrl)) {
                    ...
                    "Credentials for new user are not the same on all nodes of cluster $currUrl"
                }
     ...
     * or return false.
     */
    private suspend fun checkCredentialsSameOnAllNodes(
        credentials: UsernamePassword,
        clusterUrl: String,
    ): Boolean {
        val configurationClient =
            CouchDbConfigurationClientImpl(
                url = clusterUrl,
