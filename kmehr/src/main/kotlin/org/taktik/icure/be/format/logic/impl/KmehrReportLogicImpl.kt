	...
	override suspend fun doImport(language: String, doc: Document, hcpId: String?, protocolIds: List<String>, formIds: List<String>, planOfActionId: String?, ctc: Contact, encKeys: List<String>, cachedAttachment: ByteArray?): Contact? {
		...
		val subContactsAndServices = msg?.folders?.flatMap { f ->
			f.transactions.filter { t ->
				...
				val textItems = t.headingsAndItemsAndTexts.filterIsInstance<TextType>()
				val s = Service(
					id = uuidGen.newGUID().toString(),
					content = mapOf(language to Content(stringValue = t.headingsAndItemsAndTexts.filterIsInstance<TextType>().joinToString(separator = "\n") { it.value })),
					label = "Protocol",
				...
				val docServices = t?.headingsAndItemsAndTexts?.filterIsInstance<LnkType>()?.map { lnk ->
					Service(
						id = uuidGen.newGUID().toString(),
						content = mapOf(
							language to Content(
									...
									documentLogic.createDocument(
										...
										documentLogic.updateAttachments(
											...
											mainAttachmentChange = DataAttachmentChange.CreateOrUpdate(
												...
												utis = utis.takeIf { x -> x.isNotEmpty() }?.map { x -> x.identifier } ?: listOf("com.adobe.pdf"),
				...
				SubContact(
					...
					formId = protocolIds.indexOf(protocolId).let { formIds[it] },
