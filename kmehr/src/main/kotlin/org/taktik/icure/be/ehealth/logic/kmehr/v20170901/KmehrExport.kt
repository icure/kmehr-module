...
open class KmehrExport(
	...
	suspend fun makePerson(p: Patient, config: Config): PersonType =
		makePersonBase(p, config).apply {
			...
			usuallanguage =
				if (listOf(Config.Format.SUMEHR, Config.Format.MEDICATIONSCHEME, Config.Format.DIARYNOTE).contains(config.format)) config.defaultLanguage
				else p.languages.firstOrNull()
		...
		ItemType().apply {
			...
			lifecycle = LifecycleType().apply {
				cd = CDLIFECYCLE().apply {
					s = "CD-LIFECYCLE"
					value = when {
						(ServiceStatus.isIrrelevant(he.status) && !ServiceStatus.isActive(he.status)) || he.closingDate.isBeforeNow() -> CDLIFECYCLEvalues.INACTIVE
						he.tags.any { t -> t.type == "CD-LIFECYCLE" } -> CDLIFECYCLEvalues.fromValue(he.tags.first { t -> t.type == "CD-LIFECYCLE" }.code)
						else -> CDLIFECYCLEvalues.ACTIVE
					}
				}
			}
