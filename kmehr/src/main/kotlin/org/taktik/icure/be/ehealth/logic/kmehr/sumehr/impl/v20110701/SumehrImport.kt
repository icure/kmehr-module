...
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDADDRESSschemes
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDCONTENTschemes
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDHCPARTYschemes
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDITEMschemes
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDSEXvalues
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDTELECOMschemes
...
class SumehrImport(
    ...
    suspend fun importSumehr(
        ...
        return kmehrMessage.folders.fold(listOf()) { allRes, folder ->
            ...
            createOrProcessPatient(folder.patient, author, res, saveToDatabase, dest)?.let { patient ->
                ...
                folder.transactions.forEach { trn ->
                    val ctc: Contact = parseSumehr(trn, author, res, language, mappings, saveToDatabase)
                    if (saveToDatabase) {
    ...
    suspend fun importSumehrByItemId(
        ...
        kmehrMessage.folders.forEach { folder ->
            ...
            createOrProcessPatient(folder.patient, author, res, saveToDatabase, dest)?.let { patient ->
                res.patient = patient
                folder.transactions.forEach { trn ->
                    val ctc: Contact = parseSumehr(trn, author, res, language, mappings, saveToDatabase, itemId)
                    contactLogic.createContact(ctc)
                    res.ctcs.add(ctc)
                }
            }
    ...
    private suspend fun parseGenericTransaction(
        ...
        val contactDate = trn.date?.let { Utils.makeFuzzyLongFromDateAndTime(it, trn.time) }
            ?: trn.findItem { it: ItemType -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "encounterdatetime" } }?.let {
                it.contents?.find { c -> c.date != null }?.let { d -> Utils.makeFuzzyLongFromDateAndTime(d.date, d.time) }
            }
        val items = if (itemId?.isNotBlank() == true && itemId.isNotEmpty()) {
            ...
            if (idList.count() > 1) {
                // headings and items
                trn.findItemsByHeadingId(null, idList[0]).filter { type ->
                    type.ids.any { it.s == IDKMEHRschemes.ID_KMEHR && it.value == idList[1] }
                }
            } else {
                // only items
                trn.findItems().filter { type ->
                    type.ids.any { it.s == IDKMEHRschemes.ID_KMEHR && it.value == idList[0] }
                }
            }
        } else {
            trn.findItems()
        ...
        val (hes, svcs, sctcs) = items.fold(Triple(listOf<HealthElement>(), listOf<Service>(), listOf<SubContact>())) { (hes, svcs, sctcs), item ->
            ...
            when {
                (listOf("healthcareelement", "allergy", "adr", "risk", "socialrisk").contains(cdItem)) -> {
                    val he = parseHealthcareElement(
                        mapping?.tags?.find { it.type == "CD-ITEM" }?.code
                            ?: cdItem,
                        label,
                        item,
                        author,
                    ...
                    val service = parseGenericItem(mapping?.tags?.find { it.type == "CD-ITEM" }?.code ?: cdItem, label, item, author, language)
                    service?.let { Triple(hes, svcs + it, sctcs + SubContact(id = idGenerator.newGUID().toString(), services = listOf(ServiceLink(it.id)))) }
                        ?: Triple(hes, svcs, sctcs)
                }
            }
        }
        v.hes.addAll(if (saveToDatabase) hes.map { healthElementLogic.createEntities(listOf(it)).firstOrNull() ?: throw (IllegalStateException("Cannot save to database")) } else hes)

        return Contact(
            id = contactId,
            author = author.id,
            responsible = trn.author?.hcparties
                ?.filter { it.cds.any { cds -> cds.s == CDHCPARTYschemes.CD_HCPARTY && cds.value == "persphysician" } }
            ...
            location =
                trn.findItem { it: ItemType -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "encounterlocation" } }
                    ?.let {
                        it.contents?.flatMap { content -> content.texts.map { t -> t.value } }?.joinToString(",")
                    },

            tags = trn.findItem { it: ItemType -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "encountertype" } }?.let {
                it.contents?.mapNotNull { contentType ->
                    contentType.cds?.find { cds -> cds.s == CDCONTENTschemes.CD_ENCOUNTER }
                        ?.value?.let { v -> CodeStub.from("CD-ENCOUNTER", v, "1.0") }
                }?.toSet()
            } ?: setOf(),
            services = svcs.toSet(),
    ...
    private fun extractTags(item: ItemType): Collection<CodeStub> =
        (item.cds.filter { it.s == CDITEMschemes.CD_PARAMETER || it.s == CDITEMschemes.CD_LAB || it.s == CDITEMschemes.CD_TECHNICAL }.map { CodeStub.from(it.s.value(), it.value, it.sv) } +
            item.contents.filter { (it.cds?.size ?: 0) > 0 }.flatMap { contentType ->
                contentType.cds.filter { content ->
                    listOf(CDCONTENTschemes.CD_LAB).contains(content.s)
                }.map { CodeStub.from(it.s.value(), it.value, it.sv) }
            }).toSet()
    ...
    private fun parseGenericItem(
                ...
                Content(
                    medicationValue = Medication(
                        substanceProduct = item.contents.firstOrNull { it.substanceproduct != null }?.let { contentType ->
                            contentType.substanceproduct?.let { product ->
                                Substanceproduct(
                                    intendedcds = product.intendedcd?.let { cd -> listOf(CodeStub.from(cd.s, cd.value, cd.sv)) }
                                        ?: listOf(),
                        ...
                        medicinalProduct = item.contents.firstOrNull { it.medicinalproduct != null }?.let { contentType ->
                            contentType.medicinalproduct?.let { product ->
                                Medicinalproduct(
                                    intendedcds = product.intendedcd?.let { cd -> listOf(CodeStub.from(cd.s.toString(), cd.value, cd.sv)) }
                                        ?: listOf(),
                                    intendedname = product.intendedname.toString(),
                                )
                            }
                        },
                        compoundPrescription = item.contents.firstNotNullOfOrNull {
                            // TODO: redo this
            ...
            (item.contents.any { it.texts.any { t -> t.value?.isNotBlank() ?: false } }) -> {
                ...
                if (cdItem == "CD-PARAMETER") {
                    ...
                    item.contents.filter { (it.texts?.size ?: 0) > 0 }.flatMap {
                        it.texts.map { text ->
                            text.value?.let { v ->
                                val unit = v.replace(Regex("[0-9.,] *"), "")
                                val value = v.replace(Regex("([0-9.,]) *.*"), "$1")
        ...
        return content?.let {
            Service(
                ...
                status = (
                    (
                        item.lifecycle?.cd?.value?.value()?.let { value ->
                            when (value) {
    ...
    private suspend fun createOrProcessHcp(p: HcpartyType, saveToDatabase: Boolean): HealthcareParty? {
        ...
        return nihii
            ?.let {
                ...
                healthcarePartyLogic.listHealthcarePartiesBySsin(it).firstOrNull()
            } ?: try {
                    copyFromHcpToHcp(
                        p,
                        HealthcareParty(
        ...
        return dest ?: niss?.let {
            ...
            if (it.isNotEmpty()) {
                patientLogic.getPatients(it).filter { patient ->
                    patient.dateOfBirth?.let { birthDate -> birthDate == Utils.makeFuzzyIntFromXMLGregorianCalendar(p.birthdate.date) }
                        ?: false
                }.firstOrNull()
            } else null
        }
    ...
    private fun copyFromPersonToPatient(p: PersonType, patient: Patient, force: Boolean = true): Patient =
        patient.copy(
            ...
            addresses = patient.addresses + (
                p.addresses?.let {
                    it.map { type ->
                        val addressType = type.cds
                            .find { cds -> cds.s == CDADDRESSschemes.CD_ADDRESS }
                            ?.let { a -> AddressType.valueOf(a.value) }
                        Address(
                            addressType = addressType,
                            street = type.street,
                            city = type.city,
                            houseNumber = type.housenumber,
                            postboxNumber = type.postboxnumber,
                            postalCode = type.zip,
                            country = type.country?.cd?.value,
                            telecoms = p.telecoms
                                .filter { t -> t.cds.find { cds -> cds.s == CDTELECOMschemes.CD_ADDRESS }?.let { a -> AddressType.valueOf(a.value) } == addressType }.mapNotNull { tt ->
                                    tt.cds.find { cds -> cds.s == CDTELECOMschemes.CD_TELECOM }?.let { t -> TelecomType.valueOf(t.value) }?.let { telecomType ->
                                        Telecom(telecomType = telecomType, telecomNumber = tt.telecomnumber)
                                    }
                                },
                        )
...
private fun TransactionType.findItem(predicate: ((ItemType) -> Boolean)? = null): ItemType? = selector(this.headingsAndItemsAndTexts, predicate).firstOrNull()

private fun TransactionType.findItemsByHeadingId(predicate: ((ItemType) -> Boolean)? = null, headingId: String): List<ItemType> {
    val hits = this.headingsAndItemsAndTexts.filter {
        it is HeadingType && it.ids.any { id -> id.value == headingId }
    }

    return selector(hits.toMutableList(), predicate)
}

private fun TransactionType.findItems(predicate: ((ItemType) -> Boolean)? = null): List<ItemType> = selector(this.headingsAndItemsAndTexts, predicate)
