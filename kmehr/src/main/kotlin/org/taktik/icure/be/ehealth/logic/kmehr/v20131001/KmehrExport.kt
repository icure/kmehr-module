...
open class KmehrExport(
    ...
    private val codesMap = hashMapOf<String, Cache.ValueWrapper>()
    ...
    suspend fun createParty(m: HealthcareParty, cds: List<CDHCPARTY>? = listOf()): HcpartyType {
        return HcpartyType().apply {
            m.nihii?.let { nihii ->
                if (isNihiiValid(nihii) && nihii.isNotEmpty()) {
    ...
    private fun isNihiiValid(nihii: String) = nihii.isNotEmpty() && nihii.length == 11 &&
        (
    ...
    suspend fun makePerson(p: Patient, config: Config): PersonType {
        return makePersonBase(p, config).apply {
            ...
            if (config.format == Config.Format.SMF || config.format == Config.Format.PMF) {
                if (!p.note.isNullOrBlank()) {
                    p.note.let { note ->
                        texts.add(
                            TextType().apply {
                                value = note; l = config.defaultLanguage ?: "en"
                            },
                        )
                    }
                }
                if (!p.administrativeNote.isNullOrBlank()) {
                    p.administrativeNote.let { note ->
                        texts.add(
                            TextType().apply {
                                value = note; l = config.defaultLanguage ?: "en"
                            },
    ...
    open fun createItemWithContent(he: HealthElement, idx: Int, cdItem: String, contents: List<ContentType>, localIdName: String = "iCure-HealthElement"): ItemType? {
        return ItemType().apply {
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.ID_KMEHR; sv = "1.0"; value = idx.toString() })
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.LOCAL; sl = localIdName; sv = kmehrConfiguration.kmehrVersion; value = he.healthElementId })
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.LOCAL; sl = "icure-id"; sv = kmehrConfiguration.kmehrVersion; value = he.id })
            cds.add(CDITEM().apply { s(CDITEMschemes.CD_ITEM); value = cdItem })
            ...
            lifecycle = LifecycleType().apply {
                cd = CDLIFECYCLE().apply {
                    ...
                    value = if (((he.status and 1) == 1) || (he.closingDate != null && he.closingDate != 0L && he.closingDate!! < FuzzyValues.getCurrentFuzzyDateTime(ChronoUnit.SECONDS))) {
                        CDLIFECYCLEvalues.INACTIVE
                    } else {
            ...
            endmoment = he.closingDate?.let {
                if (it == 0L) {
                    Utils.makeMomentTypeFromFuzzyLong(FuzzyValues.getFuzzyDate(LocalDateTime.now(), ChronoUnit.DAYS))
                } else {
    ...
    /**
     * Creates a XML date from a timestamp.
    ...
    private suspend fun mapToCountryCode(country: String?): String? {
        ...
        return (codesMap[key]?.get() as String?) ?: CodeStub.from("CD-FED-COUNTRY", country.lowercase(), "1").let {
