...
class SoftwareMedicalFileImport(
		...
		ObjectMapper().let { om ->
			val txt =
				this.javaClass.classLoader
					.getResourceAsStream("org/taktik/icure/be/ehealth/logic/kmehr/smf/impl/smf.labels.json")
					?.readBytes()
					?.toString(Charsets.UTF_8) ?: "{}"
			om.readValue(txt)
		}
	private val heItemTypes: List<String> =
		listOf(
			"healthcareelement",
			"adr",
			"allergy",
		...
		return kmehrMessage.folders.fold(emptyList()) { importResults, folder ->
			...
			val insurabilities =
				folder.transactions
					?.flatMap {
						it.findItems { item ->
							item.cds.find { cds -> cds.s == CDITEMschemes.CD_ITEM }?.value == "insurancystatus"
						}
					}?.firstOrNull()
					?.let {
					...
					res.patient?.let {
						it.copy(
							patientHealthCareParties =
								it.patientHealthCareParties +
									res.hcps
										.distinctBy { hcp -> hcp.id }
										.map { hcp ->
	...
	private suspend fun parsePharmaceuticalPrescription(
		...
		val contactId =
			transactionMfid?.let { kmehrIndex.transactionIds[it]?.first?.toString() } ?: idGenerator
				.newGUID()
				.toString()
		val formId = kmehrIndex.formIdMask.xor(UUID.fromString(contactId)).toString()

		val serviceAndSubContacts =
			trn
				.findItems { it: ItemType -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "medication" } }
				.map { item ->
					val mfId = getItemMFID(item)
					val service =
						parseGenericItem("treatment", "Prescription", item, author, trnAuthorHcpId, language, kmehrIndex)
				...
				simplifiedSubContacts
					...
					.map { newFormId ->
						Form(
							id = newFormId,
							parent =
								kmehrIndex.transactionChildOf[transactionMfid]
									?.firstOrNull()
									?.let {
										kmehrIndex.transactionIds[it]?.first?.let { cid ->
											kmehrIndex.formIdMask.xor(cid).toString()
										}
		...
		return Contact(
			...
			responsible = trnAuthorHcpId,
			services =
				serviceAndSubContacts.map { it.first }.toSet() +
					(
						transactionMfid?.let {
							kmehrIndex.parentOf[it]
								?.flatMap { id ->
									kmehrIndex.transactionIds[id]
										?.second
										?.let { transactionType ->
											parseTransaction(
												transactionType,
	 ...
	 * @return the HCP id.
	 */
	private suspend fun extractTransactionAuthor(
		trn: TransactionType,
		saveToDatabase: Boolean,
		author: User,
		v: ImportResult,
		...
		?.hcparties
		?.filter { it.cds.any { cds -> cds.s == CDHCPARTYschemes.CD_HCPARTY } }
		?.firstNotNullOfOrNull {
			createOrProcessHcp(it, saveToDatabase, v)
		}?.id ?: author.healthcarePartyId
	?: throw IllegalArgumentException("The author's healthcarePartyId must be set")
	...
	private fun extractTransactionDateTime(trn: TransactionType) =
		trn
			.findItem { it.cds.any { cds -> cds.s == CDITEMschemes.CD_ITEM && cds.value == "encounterdatetime" } }
			?.let {
				it.contents
					?.find { c -> c.date != null }
					?.let { c -> Utils.makeFuzzyLongFromDateAndTime(c.date, c.time) }
	...
	private fun makeSubContact(
		...
		return when {
			!relatedItemIds.isNullOrEmpty() ->
				relatedItemIds.map { (heOrHcaMfid, heOrHcaPair) ->
					val item = heOrHcaPair.second
					if (item.cds.find { it.s == CDITEMschemes.CD_ITEM }?.value == "healthcareapproach") {
						val heId =
							kmehrIndex.approachFor[heOrHcaMfid]?.firstNotNullOfOrNull { kmehrIndex.itemIds[it] }?.first
						SubContact(
							id =
								UUID
									.nameUUIDFromBytes(("$contactId|$heId|${heOrHcaPair.first}|null").toByteArray())
									.toString(),
							formId = formId,
						...
						SubContact(
							id =
								UUID
									.nameUUIDFromBytes(("$contactId|null|${heOrHcaPair.first}|null").toByteArray())
									.toString(),
							formId = formId,
	...
	private suspend fun parseDocumentInTransaction(
		...
		val services =
			trn.headingsAndItemsAndTexts
				...
				?.map { lnk ->
					val docName =
						trn.cds.firstOrNull { it.s == CDTRANSACTIONschemes.CD_TRANSACTION }?.dn ?: trnTypeCd
						?: "unnamed_document"
					...
					Service(
						...
						qualifiedLinks =
							transactionMfid
								?.let { id ->
									kmehrIndex.itemIds[id]
										?.first
										?.toString()
										?.let {
											mapOf(
												LinkQualification.relatedService to
													mapOf(
											...
											Document(
												...
												responsible =
													trn.author
														...
														?.filter {
															it.cds.any { cd ->
																cd.s == CDHCPARTYschemes.CD_HCPARTY &&
																	cd.value == "persphysician"
															}
														}?.firstNotNullOfOrNull {
															createOrProcessHcp(it, saveToDatabase, v)
														}?.id ?: author.healthcarePartyId,
														...
														documentLogic.updateAttachments(
															...
															mainAttachmentChange =
																DataAttachmentChange.CreateOrUpdate(
																	data = flowOf(DefaultDataBufferFactory.sharedInstance.wrap(lnk.value)),
																	size = lnk.value.size.toLong(),
																	utis = listOf(mainUti) + otherUtis,
		...
		val simplifiedSubContacts = simplifySubContacts(subContacts.flatten()).toSet()
		if (simplifiedSubContacts.isNotEmpty()) {
			v.forms.addAll(
				simplifiedSubContacts
					.filter { sc -> !v.forms.any { it.id == sc.formId } && sc.services.isNotEmpty() }
	...
	private fun extractUtis(lnk: LnkType): Pair<String, Set<String>> {
		...
		return (utis.firstOrNull()?.identifier ?: "com.adobe.pdf").let {
			val otherUtis =
				(if (utis.size > 1) utis.subList(1, utis.size).map { u -> u.identifier } else listOf()).toSet()
	...
	private suspend fun parseGenericTransaction(
				...
				.filter { item ->
					...
					if (cdItem == "healthcareelement") {
						trnItems.none { checkItem ->
							...
							checkCdItem != "healthcareelement" && heItemTypes.contains(checkCdItem) &&
								isHealthElementTypeEqual(
									item,
									checkItem,
								)
					...
					when (val cdItem = tags.find { it.type == "CD-ITEM" }?.code ?: "note") {
						...
						"encountertype", "encounterdatetime", "encounterlocation" ->
							Pair(
								svcs,
								sbctcs,
							) // already added at contact level
						...
						"incapacity" ->
							...
							Pair(
								svcs + service,
								makeSubContact(contactId, formId, mfId, service, kmehrIndex).let { sbctcs + it },
							)
						}
					}
				}
		...
		return Contact(
			...
			descr =
				trn.headingsAndItemsAndTexts
					.filterIsInstance<TextType>()
					.firstOrNull()
					?.value,
			location =
				trn
					.findItem { it -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "encounterlocation" } }
					?.let {
						it.contents?.flatMap { c -> c.texts.map { t -> t.value } }?.joinToString(",")
					},
			encounterType =
				trn
					.findItem { it -> it.cds.any { it.s == CDITEMschemes.CD_ITEM && it.value == "encountertype" } }
					?.let {
	...
	private fun mapItem(
		...
		mappings: Map<String, List<ImportMapping>>,
		language: String,
	): Pair<String, Set<CodeStub>> {
		val guessedCdItem =
			(item.cds.find { it.s == CDITEMschemes.CD_ITEM }?.value ?: "note").let { cdItem ->
				when {
					item.contents.any {
						it.cds.any { cds ->
							cds.s == CDCONTENTschemes.LOCAL && cds.sl == "MEDINOTE.MEDICALCODEID"
						}
					} -> "acts"
					else -> cdItem
				}
		...
		val mapping =
			mappings[guessedCdItem]?.find {
				(
					(it.lifecycle ?: "*") == "*" || it.lifecycle ==
						item.lifecycle
							?.cd
							?.value
							?.value()
		...
		val label =
			item.cds.find { it.s == CDITEMschemes.LOCAL && it.sl == "org.taktik.icure.label" }?.value
				...
				?: "Note"
		return label to (
			setOf(CodeStub.from("CD-ITEM", cdItem, "1")) + (
				mapping?.tags?.filter { it.type != "CD-ITEM" }
					?: setOf()
				)
			)
	}
	 ...
	 */
	private fun isHealthElementTypeEqual(
		item: ItemType,
		checkItem: ItemType,
	 ...
	 */
	private fun parseHealthcareApproach(
		cdItem: String = "healthcareapproach",
		label: String,
		item: ItemType,
		author: User,
		...
		return PlanOfAction(
			...
			responsible = trnAuthorHcpId,
			tags =
				setOf(CodeStub.from("CD-ITEM", cdItem, "1")) + extractTags(item) + (
				...
				Insurability(
					insuranceId =
						if (it.id.s == IDINSURANCEschemes.ID_INSURANCE) {
							insuranceLogic
								.listInsurancesByCode(it.id.value)
								.firstOrNull()
								?.id
						} else {
							null
	 ...
	 */
	private fun isMedication(service: Service): Boolean = service.content.values.any { it.medicationValue != null }
	...
	private suspend fun parseIncapacity(
		...
		val ittform =
			Form(
				id = formId,
				formTemplateId =
					getFormTemplateIdByGuid(
						author,
						"FFFFFFFF-FFFF-FFFF-FFFF-INCAPACITY00",
					),
				// ITT form template
				parent =
					transactionMfid?.let { tmfId ->
		...
		val mapServ =
			mapOf(
				"incapacité de" to
					item.contents
						.find { it.incapacity != null }
						?.let { content ->
							// TODO Dorian fix that
							content.incapacity.cds
								.filterIsInstance<CDINCAPACITY>()
								.map { it.value }
						}?.let {
							Pair(
				...
				"pour cause de" to
					item.contents
						...
						?.let {
							// TODO Dorian fix that
							it.incapacity.incapacityreason
								?.cd
								?.value
						}?.let {
							Pair(
							...
							Service(
								...
								qualifiedLinks =
									mapOf(
										LinkQualification.relatedService to
											mapOf(
												UUID.randomUUID().toString() to mainServiceId,
											),
									),
								created =
									item.recorddatetime
								...
								content =
									(it as? Pair<Content, List<CodeStub>>)?.let { content -> mapOf(language to content.first) }
										?: (it as? Content)?.let { content -> mapOf(language to content) } ?: mapOf(),
								tags =
									setOf(CodeStub.from("CD-ITEM", "incapacity", "1")) + (
		...
		return Triple(
			servicesAndSubContacts.map { it.first },
			simplifySubContacts(servicesAndSubContacts.flatMap { it.second }),
			ittform,
		)
	}
	...
	private fun parseHealthcareElement(
		...
		return HealthElement(
			id = idGenerator.newGUID().toString(),
			healthElementId =
				mfId?.let { kmehrIndex.itemIds[it]?.first?.toString() } ?: idGenerator
					.newGUID()
					.toString(),
			descr = getItemDescription(item, label),
			...
			plansOfAction =
				kmehrIndex.approachFor
					.filter { (_, v) -> v.contains(mfId) }
					.mapNotNull { (k, _) ->
					...
					.flatMap { contentType ->
						contentType.cds
							...
							contentType.cds
								...
								}.map { CodeStub.from("BE-THESAURUS", it.value, it.sv) } +
							contentType.cds
								.filter {
	...
	private fun extractTags(item: ItemType): Collection<CodeStub> =
		(
			item.cds
				.filter {
					it.s == CDITEMschemes.CD_PARAMETER || it.s == CDITEMschemes.CD_LAB || it.s == CDITEMschemes.CD_TECHNICAL ||
						it.s == CDITEMschemes.CD_CONTACT_PERSON
				}.map { CodeStub.from(it.s.value(), it.value, it.sv) } +
				item.cds
	...
	private fun parseGenericItem(
		...
		val tags =
			setOf(CodeStub.from("CD-ITEM", cdItem, "1")) + extractTags(item) + (
				item.temporality
					?.cd
					?.value
					?.let { setOf(CodeStub.from("CD-TEMPORALITY", it.value(), "1")) }
					?: setOf()
				) + (
		...
		return Service(
			...
			label =
				tags.find { it.type == "CD-PARAMETER" }?.let {
							...
							kmehrIndex.itemIds[itemId]
								...
								?.let { itt ->
									mapOf(
										LinkQualification.relatedService to
											mapOf(
												UUID.randomUUID().toString() to itt,
											),
									)
								}
						}
			...
			content =
				when {
					(
						item.contents.any {
							it.substanceproduct != null || it.medicinalproduct != null || it.compoundprescription != null
						}
						) -> {
						Content(
							medicationValue =
								Medication(
									substanceProduct =
										item.contents
											.firstOrNull { it.substanceproduct != null }
											?.let { contentType ->
												contentType.substanceproduct?.let {
													Substanceproduct(
														intendedcds =
															it.intendedcd?.let { cd ->
																listOf(
																	CodeStub.from(
																		cd.s.value(),
																		cd.value,
									...
									medicinalProduct =
										item.contents
											.firstOrNull { it.medicinalproduct != null }
											?.let { contentType ->
												contentType.medicinalproduct?.let {
													Medicinalproduct(
														intendedcds =
															it.intendedcds?.map { cd ->
																CodeStub.from(
																	cd.s.value(),
																	cd.value,
																	cd.sv,
									...
									compoundPrescription =
										item.contents
											.firstOrNull {
												...
												contentType.compoundprescription
													?.content
													?.mapNotNull {
														// spec is unclear, some software put text in <magistraltext> some put it directly in compoundprescription
														// try to detect each case
					...
					(item.contents.any { it.decimal != null }) ->
						item.contents.firstOrNull { it.decimal != null }?.let {
					...
					(item.contents.any { it.isBoolean != null }) ->
						item.contents
							.firstOrNull { it.isBoolean != null }
							?.let {
	...
	private fun ItemType.hasContentOfType(content: String?): Boolean =
		content?.let { c ->
			c == "m" &&
				this.contents.any {
					it.medicinalproduct != null || it.substanceproduct != null || it.compoundprescription != null
				} ||
				c == "s" &&
				this.contents.any {
					(it.texts?.size ?: 0) > 0 || (it.cds?.size ?: 0) > 0 || it.hcparty != null
				}
		} ?: true
	 ...
	 * @return a HCP
	 */
	private suspend fun createOrProcessHcp(
		p: HcpartyType,
		saveToDatabase: Boolean,
		v: ImportResult,
	): HealthcareParty? {
		...
		val specialty: String? =
			p.cds
				.find { it.s == CDHCPARTYschemes.CD_HCPARTY }
				?.value
				?.trim()

		// test if already exist in current file
		val hcpBySsinOrNihii =
			v.hcps.find {
				...
				?: nihii
					?.let { healthcarePartyLogic.listHealthcarePartiesByNihii(it).firstOrNull() }
					?.also { v.hcps.add(it) }
				?: niss?.let { healthcarePartyLogic.listHealthcarePartiesBySsin(it).firstOrNull() }?.also { v.hcps.add(it) }

		val existing =
			if (hcpBySsinOrNihii == null && ((nihii == null || nihii.trim() == "") && (niss == null || niss.trim() == "")) &&
		...
		return existing
			?: try {
				copyFromHcpToHcp(
					p,
					HealthcareParty(id = idGenerator.newGUID().toString(), nihii = nihii, ssin = niss),
				).also {
					v.hcps.add(it)
							...
							Address(
								...
								telecoms =
									p.telecoms
										.filter { t ->
											t.cds
												.find { it.s == CDTELECOMschemes.CD_ADDRESS }
												?.let { AddressType.valueOf(it.value) } == addressType
										}.mapNotNull { t ->
											t.cds
												.find { it.s == CDTELECOMschemes.CD_TELECOM }
												?.let { TelecomType.valueOf(it.value) }
												?.let { telecomType ->
	...
	private suspend fun checkIfPatientExists(
		...
		res.ssin = niss ?: ""
		res.dateOfBirth = p.birthdate?.date?.let { Utils.makeFuzzyIntFromXMLGregorianCalendar(it) }
	...
	private suspend fun getExistingPatient(
		...
		val niss =
			validSsinOrNull(p.ids.find { it.s == IDPATIENTschemes.ID_PATIENT }?.value) // searching empty niss return all patients
		v.notNull(niss, "Niss shouldn't be null for patient $p")
		...
		return dest ?: niss?.let {
		...
		} ?: p.birthdate?.date?.let { patDob ->
			patientLogic
				.matchEntitiesBy(
					PatientByHcPartyDateOfBirthFilter(
						healthcarePartyId = checkNotNull(author.healthcarePartyId) { "HealthcareParty id cannot be null" },
						dateOfBirth =
							Utils.makeFuzzyIntFromXMLGregorianCalendar(patDob)
			...
			?.let {
				patientLogic
					.getPatients(it).firstOrNull { patient ->
						patient.dateOfBirth?.let { dateOfBirth ->
							dateOfBirth ==
								p.birthdate?.date?.let {
									Utils.makeFuzzyIntFromXMLGregorianCalendar(
										it,
									)
								}
						} ?: false
	...
	) = getExistingPatientWithHcpHierarchy(
		p,
		author,
		v,
		dest,
	)?.let { it.copy(insurabilities = it.insurabilities + insurabilities) }
		?: Patient(
			id = idGenerator.newGUID().toString(),
			insurabilities = insurabilities,
			delegations =
				author.healthcarePartyId?.let { mapOf(it to setOf()) }
					?: mapOf(),
		).let {
	 ...
	 * @return a Patient
	 */
	private fun copyFromPersonToPatient(
		p: PersonType,
		patient: Patient,
		force: Boolean = true,
	): Patient =
		patient.copy(
			...
			dateOfBirth = p.birthdate?.date?.let { Utils.makeFuzzyIntFromXMLGregorianCalendar(it) },
			ssin =
			...
			dateOfDeath =
				if (force || patient.dateOfDeath == null) {
					p.deathdate?.let {
						Utils.makeFuzzyIntFromXMLGregorianCalendar(
							it.date,
						)
			...
			externalId =
				p.ids.firstOrNull { i -> i.s == IDPATIENTschemes.LOCAL && i.sl == "PatientReference" }?.value?.let { patRef ->
					if (force || patient.externalId == null) patRef else patient.externalId
				} ?: patient.externalId,
			alias =
				p.ids.firstOrNull { i -> i.s == IDPATIENTschemes.LOCAL && i.sl == "PatientAlias" }?.value?.let { alias ->
								...
								telecoms =
									p.telecoms
										.filter { t ->
											...
											it.cds
												.find { cds -> cds.s == CDTELECOMschemes.CD_TELECOM }
	...
	private val consultationFormMeasureLabels: Map<String, String> =
		mapOf(
	...
	private suspend fun getFormTemplateIdByGuid(
		author: User,
		guid: String,
	): String? = formTemplateLogic.getFormTemplatesByGuid(author.id, "deptgeneralpractice", guid).firstOrNull()?.id

	data class HeVersionType(
		val he: HealthElement,
		val mfId: String,
		val isANewVersionOfId: String?,
		var versionId: String?,
	)

	data class DocumentLinkType(
		val document: Document,
		val service: Service,
		val isAChildOfId: String?,
	)
 ...
 * @return the MF-ID, if any.
 */
fun getItemMFID(item: ItemType): String? = item.ids.find { it.s == IDKMEHRschemes.LOCAL && it.sl == "MF-ID" }?.value
 ...
 * @return the MF-ID, if any.
 */
fun getTransactionMFID(trn: TransactionType): String? = trn.ids.find { it.s == IDKMEHRschemes.LOCAL && it.sl == "MF-ID" }?.value
...
data class KmehrMessageIndex(
	...
	val formIdMask: UUID =
		UUID
			.randomUUID()
			.xor(UUID.randomUUID()),
	// Ensure that marker bits are set to 0 by xoring two UUIDs
) {
	fun isChildTransaction(trn: TransactionType?) = trn?.let { getTransactionMFID(it)?.let { mfid -> childOf.containsKey(mfid) } } ?: false

	fun children(trn: TransactionType?) = trn?.let { getTransactionMFID(it)?.let { mfid -> parentOf[mfid] } } ?: listOf()
}
...
fun simplifySubContacts(scts: Collection<SubContact>): Collection<SubContact> =
	scts
		.groupBy { it.id }
		.mapValues { it.value.first().copy(services = it.value.flatMap { svc -> svc.services }) }
		.values
 ...
 */
fun Kmehrmessage.performIndexation(idGenerator: UUIDGenerator) =
	this.folders.fold(KmehrMessageIndex()) { folderKmi, folder ->
		folder.transactions.fold(folderKmi) { transactionKmi, trn ->
			...
			trn.findItems().fold(
				transactionKmi.copy(
					transactionIds =
						tmfId?.let { transactionKmi.transactionIds + (it to (idGenerator.newGUID() to trn)) }
							?: transactionKmi.transactionIds,
					transactionChildOf =
						if (tmfId != null && !childOfTLinks.isNullOrEmpty()) {
						...
						if (tmfId != null && !childOfTLinks.isNullOrEmpty()) {
							transactionKmi.transactionParentOf +
								childOfTLinks.map {
									it.second to (
										listOf(tmfId) + (
											transactionKmi.transactionParentOf[it.second]
												?: listOf()
											)
										)
								}
				...
				val links =
					item.lnks
						.filter {
							(
								it.type == CDLNKvalues.ISASERVICEFOR || it.type == CDLNKvalues.ISATTESTATIONOF ||
									it.type == CDLNKvalues.ISACHILDOF ||
									it.type == CDLNKvalues.ISAPPROACHFOR
								) &&
								it.url != null
						}.mapNotNull { lnk ->
							extractMFIDFromUrl(lnk.url)?.let { lnk.type to it }
				...
				kmi.copy(
					...
					serviceFor =
						if (mfId != null && !serviceForLinks.isNullOrEmpty()) {
							kmi.serviceFor + (mfId to serviceForLinks.map { it.second })
						} else {
							kmi.serviceFor
						},
					childOf =
						if (mfId != null && !childOfLinks.isNullOrEmpty()) {
					...
					parentOf =
						if (mfId != null && !childOfLinks.isNullOrEmpty()) {
							kmi.parentOf +
								childOfLinks.map {
									it.second to (listOf(mfId) + (kmi.parentOf[it.second] ?: listOf()))
								}
						} else {
							kmi.parentOf
						},
					approachFor =
						if (mfId != null && !approachForLinks.isNullOrEmpty()) {
							kmi.approachFor + (mfId to approachForLinks.map { it.second })
						} else {
							kmi.approachFor
						},
					attestationOf =
						if (mfId != null && !attestationOfLinks.isNullOrEmpty()) {
...
private fun selector(
	...
	headingsAndItemsAndTexts.fold(listOf()) { acc, it ->
		when (it) {
...
private fun TransactionType.findItem(predicate: ((ItemType) -> Boolean)? = null): ItemType? =
	selector(this.headingsAndItemsAndTexts, predicate).firstOrNull()

private fun TransactionType.findItems(predicate: ((ItemType) -> Boolean)? = null): List<ItemType> =
	selector(this.headingsAndItemsAndTexts, predicate)
