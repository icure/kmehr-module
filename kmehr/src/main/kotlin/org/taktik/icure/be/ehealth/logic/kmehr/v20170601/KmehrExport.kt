...
open class KmehrExport(
    ...
    val kmehrConfiguration: KmehrConfiguration
) {
        ...
        HcpartyType().apply {
            ...
            this.cds.addAll(
                when {
                    ...
                    (m.speciality ?: "") != "" -> listOf(CDHCPARTY().apply { s(CDHCPARTYschemes.CD_HCPARTY); value = m.speciality })
                    else -> listOf(CDHCPARTY().apply { s(CDHCPARTYschemes.CD_HCPARTY); value = "persphysician" })
                }
            )
    ...
    private fun makePersonBase(p: Patient, config: Config): PersonType {
        ...
        return PersonType().apply {
            ssin?.let { ssin ->
                ids.add(IDPATIENT().apply { s = IDPATIENTschemes.ID_PATIENT; sv = "1.0"; value = ssin.replace("[^0-9]".toRegex(), "") })
            }
    ...
    open fun createItemWithContent(svc: Service, idx: Int, cdItem: String, contents: List<ContentType>, localIdName: String = "iCure-Service"): ItemType? {
        return ItemType().apply {
            ...
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.LOCAL; sl = localIdName; sv = kmehrConfiguration.kmehrVersion; value = svc.id })
    ...
    open fun createItemWithContent(he: HealthElement, idx: Int, cdItem: String, contents: List<ContentType>, localIdName: String = "iCure-Service"): ItemType? {
        return ItemType().apply {
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.ID_KMEHR; sv = "1.0"; value = idx.toString() })
            ids.add(IDKMEHR().apply { s = IDKMEHRschemes.LOCAL; sl = localIdName; sv = kmehrConfiguration.kmehrVersion; value = he.healthElementId ?: he.id })
            cds.add(CDITEM().apply { s(CDITEMschemes.CD_ITEM); value = cdItem })
            ...
            lifecycle = LifecycleType().apply {
                cd = CDLIFECYCLE().apply {
                    s = "CD-LIFECYCLE"
                    value = if (ServiceStatus.isIrrelevant(he.status) || (!ServiceStatus.isActive(he.status)) || (he.closingDate ?: 0 > FuzzyValues.getCurrentFuzzyDateTime(ChronoUnit.DAYS))) {
    ...
    private fun makeTelecoms(addresses: Collection<Address>?): List<TelecomType> =
        addresses
            ?.filter { it.addressType != null }
            ?.flatMapTo(ArrayList()) { a ->
    ...
    suspend fun makeAddresses(addresses: Collection<Address>?): List<AddressType> =
        addresses
            ?.filter { it.addressType != null && it.postalCode != null && it.street != null }
            ?.mapTo(ArrayList()) { a ->
            AddressType().apply {
    ...
    /**
     * Converts a Fuzzy Date to a XML KmEHR format.
     ...
     * @param kmehrId the ID of the KmEHR.
        ...
        Kmehrmessage().apply {
            header = HeaderType().apply {
                ...
                ids.add(IDKMEHR().apply {
                    s = IDKMEHRschemes.ID_KMEHR; sv = "1.10"
                    value = kmehrId ?: ((sender.nihii ?: sender.id) + "." + (config._kmehrId ?: System.currentTimeMillis())) })
    ...
    suspend fun exportContactReport(patient: Patient, sender: HealthcareParty, recipient: HealthcareParty?, dem: PlanOfAction, ssc: Form, text: String, attachmentDocIds: List<String>, config: Config, stream: OutputStream) {
        ...
        val jaxbMarshaller = JAXBContext.newInstance("org.taktik.icure.be.ehealth.dto.kmehr.v20170601.be.fgov.ehealth.standards.kmehr.schema.v1", com.sun.xml.bind.v2.ContextFactory::class.java.classLoader).createMarshaller()
    ...
    /**
     * Creates a ID-KMEHR element from the index passed as parameter.
     * @param index the Index.
     * @return an ID-KMEHR element.
     */
