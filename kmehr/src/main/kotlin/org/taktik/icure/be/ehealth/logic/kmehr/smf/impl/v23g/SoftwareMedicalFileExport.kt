...
class SoftwareMedicalFileExport(
    ...
    private suspend fun makePatientFolder(
            ...
            TransactionType().apply {
                ...
                ids.add(
                    IDKMEHR().apply {
                        ...
                        sl = "MF-ID"
                        sv = "1.0"
                        value = UUIDGenerator().newGUID().toString()
                    },
                )
                cds.add(
                    CDTRANSACTION().apply {
                    ...
                    AuthorType().apply {
                        hcparties.add(
                            healthcarePartyLogic
                                .getHealthcareParty(
                                    patient.author?.let {
                                        userLogic.getUser(it, false)?.healthcarePartyId
                                    } ?: healthcareParty.id,
                                )?.let { createParty(it) },
                        )
                    }
        ...
        val contacts =
            getAllContacts(healthcareParty, sfks.toList()).sortedBy {
                it.openingDate
            }
        val startIndex = folder.transactions.size

        val nonConfidentialHealthElements =
            getNonConfidentialItems(getHealthElements(healthcareParty, sfks, config))
                .let {
        ...
        oldestHeByHeId =
            hesByHeIdSortedByDate.mapValues {
                it.value.first()
            }
        ...
        hesByContactId[null].orEmpty().map { he ->
            addHealthCareElement(
        ...
        contacts.forEachIndexed contactsLoop@{ index, encContact ->
            ...
            val subContacts =
                if (config.soft?.name == "Medispring") {
                    contact.subContacts.filter { subContact ->
                        subContact.healthElementId == null && subContact.planOfActionId == null &&
                            subContact.formId != null
                    }
                } else {
                    contact.subContacts.toList()
                }
            subContacts.forEachIndexed { idx, subContact ->
                folder.transactions.add(
                    TransactionType().apply {
                        ...
                        val (cdTransactionRef, defaultCdItemRef, exportAsDocument) =
                            when {
                                contact.encounterType?.code == "labresult" -> Triple("labresult", "lab", true)
                                contact.tags.any { it.type == "CD-TRANSACTION" && it.code == "labresult" } ->
                                    Triple(
                                        "labresult",
                                        "lab",
                                        true,
                        ...
                        ids.add(
                            IDKMEHR().apply {
                                s = IDKMEHRschemes.LOCAL
                                sl = "MF-ID"
                                value = transactionMFID
                            },
                        )
                        cds.add(
                            CDTRANSACTION().apply {
                                s = CDTRANSACTIONschemes.CD_TRANSACTION
                                value = cdTransactionRef
                                dn =
                                    subContact.descr ?: contact.descr
                            },
                        )
                        ...
                        isIscomplete = true
                        isIsvalidated = true
                        contact.openingDate?.let {
                            headingsAndItemsAndTexts.add(
                                makeEncounterDateTime(headingsAndItemsAndTexts.size + 1, it),
                            )
                        }
                        contact.location?.let {
                            headingsAndItemsAndTexts.add(
                                makeEncounterLocation(headingsAndItemsAndTexts.size + 1, it, language),
                            )
                        }
                        contact.encounterType?.let {
                            headingsAndItemsAndTexts.add(
                                makeEncounterType(headingsAndItemsAndTexts.size + 1, it),
                            )
                        }
                        ...
                        services.filter { s -> s.tags.find { t -> t.code == "incapacity" } != null }.forEach { incapacityService ->
                            ...
                            incapacityService.content[language]?.documentId?.let { docId ->
                                createLinkToDocument(
                                    ...
                                    language,
                                    config,
                                    parentContactId ?: transactionMFID,
                                    decryptor,
                                )
                            }
                        }
                        services.filter { s -> s.tags.find { t -> t.code == "physiotherapy" } != null }.forEach { kineService ->
                            specialPrescriptions.add(
                                makeKinePrescriptionTransaction(
                                    kineService,
                                    language,
                                    decryptor,
                                    parentContactId ?: transactionMFID,
                        ...
                        services.filter { s -> s.tags.find { t -> t.code == "medicalcares" } != null }.forEach { nurseService ->
                            specialPrescriptions.add(
                                makeNursePrescriptionTransaction(
                                    nurseService,
                                    language,
                                    decryptor,
                                    parentContactId ?: transactionMFID,
                        ...
                        if (exportAsDocument && services.size == 1) {
                            ...
                            services.forEach servicesLoop@{ svc ->
                                val specialTag =
                                    svc.tags.find { t ->
                                        listOf("incapacity", "physiotherapy", "medicalcares").contains(t.code)
                                    }
                                ...
                                if (!forSeparateTransaction) {
                                    ...
                                    val cdItem =
                                        (svcCdItem?.code ?: defaultCdItemRef).let {
                                            if (it == "parameter") {
                                    ...
                                    val contents =
                                        svc.content.entries.flatMap {
                                            makeContent(it.key, it.value)?.let { c ->
                                                listOf(
                                                    c.apply {
                                                        if (svcCdItem == null && texts.size > 0) {
                                                            if (!svc.label.isNullOrBlank() && svc.label != "<invalid>" &&
                                                                !texts.first().value.startsWith(svc.label!!)
                                                            ) {
                                        ...
                                        createItemWithContent(
                                                ...
                                                if (!svc.label.isNullOrEmpty()) {
                                                    this.cds.add(
                                                        CDITEM().apply {
                                                            ...
                                                            sv = "1.0"
                                                            dn =
                                                                if (svc.comment == "" || svc.comment == null) {
                                                                    svc.label
                                                                } else {
                                                                    svc.comment
                                                                }
                                            ...
                                            if (cdItem == "medication") {
                                                svc.content.values.find { it.medicationValue?.instructionForPatient != null }?.let {
                                                    this.posology =
                                                        ItemType.Posology().apply {
                                                            text =
                                                                TextType().apply {
                                                                    l = language
                                                                    value = it.medicationValue!!.instructionForPatient
                                                                }
                                                        }
                                                }
                                            }
                            ...
                            parentContactId?.let {
                                headingsAndItemsAndTexts.add(
                                    LnkType().apply {
                                        type = CDLNKvalues.ISACHILDOF
                                        url = makeLnkUrl(it)
                                    },
                                )
                            }
                        }
                        val subContactsWithHealthElementId =
                            contact.subContacts.filter { subContactWithHealthElementId ->
                                subContactWithHealthElementId.healthElementId != null &&
                                    subContactWithHealthElementId.formId == subContact.formId
                            }
            ...
            folder.transactions.add(
                TransactionType().apply {
                    ...
                    cds.add(
                        CDTRANSACTION().apply {
                            s = CDTRANSACTIONschemes.CD_TRANSACTION
                            value = "pharmaceuticalprescription"
                        },
                    )
                    ...
                    if (contents.isNotEmpty()) {
                        createItemWithContent(
                            ...
                            svc.formId?.let {
                                this.lnks.add(
                                    LnkType().apply {
                                        type = CDLNKvalues.ISATTESTATIONOF
                                        url = makeLnkUrl(it)
                                    },
                                )
                            }
                    ...
                    TransactionType().apply {
                        ...
                        cds.add(
                            CDTRANSACTION().apply {
                                s = CDTRANSACTIONschemes.CD_TRANSACTION
                                value = "note"
                                dn = svc.content[language]?.stringValue
                            },
                        )
                        (svc.modified ?: svc.created)?.let {
                            date = makeXGC(it)
    ...
    private fun addServiceLinkToHealthElement(contact: Contact) {
        ...
        val subContactServicesByFormId =
            subContactsByFormId.mapValues {
     ...
     * @param config the Config.
     */
    private fun addHistoryLinkAndCacheService(
        item: ItemType,
        serviceId: String,
        config: Config,
    ) {
        if (config.format != Config.Format.PMF) {
            itemByServiceId[serviceId]?.let {
                getItemMFID(it)?.let {
                    item.lnks.add(
                        LnkType().apply {
                            type = CDLNKvalues.ISANEWVERSIONOF
                            url = makeLnkUrl(it)
                        },
                    )
                }
     ...
     */
    private suspend fun makeNursePrescriptionTransaction(
        service: Service,
        language: String,
        decryptor: AsyncDecrypt?,
        transactionMfId: String,
    ): TransactionType = makeSpecialPrescriptionTransaction(service, "nursing", language, decryptor, transactionMfId)
     ...
     * @return a TransactionType.
     */
    private suspend fun makeKinePrescriptionTransaction(
        service: Service,
        language: String,
        decryptor: AsyncDecrypt?,
        transactionMfId: String,
    ): TransactionType = makeSpecialPrescriptionTransaction(service, "physiotherapy", language, decryptor, transactionMfId)
    ...
    private suspend fun makeSpecialPrescriptionTransaction(
        ...
        TransactionType().apply {
            ...
            cds.add(
                CDTRANSACTION().apply {
                    s = CDTRANSACTIONschemes.CD_TRANSACTION_TYPE
            ...
            LnkType()
                .apply {
                    type = CDLNKvalues.ISACHILDOF
                    url = makeLnkUrl(transactionMfId)
                }.also { headingsAndItemsAndTexts.add(it) }

            service.content[language]?.documentId?.let { documentId ->
                try {
                    documentLogic.getDocument(documentId)?.let { d ->
                        documentLogic
                            .getAndDecryptMainAttachment(
                                d.id,
                            ).let { headingsAndItemsAndTexts.add(makeMultimediaLnkType(d, it, decryptor)) }
                    }
                } catch (e: Exception) {
     ...
     * @return the ItemType.
     */
    private fun makeIncapacityItem(
        service: Service,
        language: String,
        index: Number = 0,
    ): ItemType {
        ...
        return ItemType().apply {
            ...
            ids.add(
                IDKMEHR().apply {
                    s = IDKMEHRschemes.LOCAL
                    sl = "MF-ID"
                    sv = kmehrConfig.kmehrVersion
                    value = service.id
                },
            )
            cds.add(
                CDITEM().apply {
                    s(CDITEMschemes.CD_ITEM)
                    value = "incapacity"
                },
            )
                    ...
                    incapacity =
                        IncapacityType().apply {
                            ...
                            (
                                ...
                                incapacityreason =
                                    IncapacityreasonType().apply {
                                        cd =
                                            CDINCAPACITYREASON().apply {
                                                value =
                                                    try {
     ...
     */
    private suspend fun makeSummaryTransaction(
        contact: Contact,
        service: Service,
        language: String,
    ): TransactionType {
        fun getCompoundValueContent(
            service: Service,
            label: String,
        ) = service.content[language]
            ?.compoundValue
        ...
        return TransactionType().apply {
            ...
            headingsAndItemsAndTexts.add(
                LnkType().apply {
                    ...
                    mediatype = mediaType
                    value =
                        content?.stringValue?.toByteArray()
                },
            )
            headingsAndItemsAndTexts.add(
                LnkType().apply {
                    type = CDLNKvalues.ISACHILDOF
                    url = makeLnkUrl(contact.id)
                },
            )
        }
    }
    ...
    private suspend fun makeMultimediaLnkType(
        ...
        val data =
            if ((
                    document.encryptionKeys.isNotEmpty() ||
                        document.securityMetadata?.secureDelegations?.any { it.value.encryptionKeys.isNotEmpty() } == true
                ) &&
                decryptor != null
            ) {
                decryptor
                    .decrypt(
                        listOf(
                            documentMapper.map(document).copy(encryptedAttachment = documentLogic.getAndDecryptMainAttachment(document.id)),
                        ),
        ...
        return LnkType().apply {
            type = CDLNKvalues.MULTIMEDIA
            mediatype = documentMediaType(document)
            value = data
        }
    }
     ...
     * @return an ItemType.
     */
    private fun makeEncounterDateTime(
        index: Int,
        yyyymmddhhmmss: Long,
    ): ItemType =
        ItemType().apply {
            ...
            contents.add(
                ContentType().apply {
                    ...
                    time =
                        makeXMLGregorianCalendarFromFuzzyLong(yyyymmddhhmmss)?.apply {
     ...
     * @return an ItemType.
     */
    private fun makeEncounterLocation(
        index: Int,
        location: String,
        language: String,
    ): ItemType =
        ...
        ItemType().apply {
            ...
            contents.add(
                ContentType().apply {
                    cds.add(
                        CDCONTENT().apply {
                            s = CDCONTENTschemes.CD_ENCOUNTER
                            value = encounterType.code
                        },
                    )
                },
            )
     ...
     */
    private suspend fun makeContactPeople(
        startIndex: Int,
        pat: Patient,
        config: Config,
    ): List<ItemType> {
        val partnersById: Map<String, Patient> =
            patientLogic
                .getPatients(pat.partnerships.mapNotNull { it.partnerId })
                .filterNotNull()
                .toList()
                .associateBy { partner -> partner.id }
            ...
            .mapIndexed { i, partnership ->
                partnersById[partnership.partnerId]?.let { partner ->
                    ItemType().apply {
                        ...
                        cds.add(
                            CDITEM().apply {
                                s(CDITEMschemes.CD_CONTACT_PERSON)
                                value = partnership.otherToMeRelationshipDescription
                            },
                        )
     ...
     * @return an ItemType.
     */
    private suspend fun makeGmdManager(
        itemIndex: Int,
        config: Config,
        hcp: HealthcareParty,
        ...
        ItemType()
            .apply {
                ...
                it.contents
                    .first()
                    .hcparty.ids
                    .filter { hcpId -> hcpId.s == IDHCPARTYschemes.ID_HCPARTY }
                    .size == 1
            }
     ...
     * @return an ItemType.
     */
    private suspend fun makeInsurancyStatus(
        itemIndex: Int,
        config: Config,
                                    ...
                                    ContentType().apply {
                                        insurance =
                                            InsuranceType().apply {
                                                id =
                                                    IDINSURANCE().apply {
                                                        s = IDINSURANCEschemes.ID_INSURANCE
                                                        value = it.code!!.substring(0, 3)
                                                    }
    ...
    private suspend fun getLastGmdManager(pat: Patient): Pair<HealthcareParty?, ReferralPeriod?> {
        ...
        val gmdRelationship =
            pat.patientHealthCareParties.find { it.referralPeriods.any(isActive) }
     ...
     */
    private suspend fun checkHcpAndCreateCorrespondingPartyType(
        hcpId: String,
        cds: List<CDHCPARTY>? = emptyList(),
    ): HcpartyType? = healthcarePartyLogic.getHealthcareParty(hcpId)?.let { createParty(it, cds) }
            ...
            cds.addAll(
                codes.map { code ->
                    when {
                        code.type == "ICPC" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.ICPC
                                sv = code.version
                                value = code.code
                            }
                        code.type == "ICD" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.ICD
                                sv = code.version
                                value = code.code
                            }
                        code.type == "CD-ATC" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.CD_ATC
                                sv = code.version
                                value = code.code
                            }
                        code.type == "CD-PATIENTWILL" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.CD_PATIENTWILL
                                sv = code.version
                                value = code.code
                            }
                        code.type == "BE-THESAURUS" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.CD_CLINICAL
                                sv = code.version
                                value = code.code
                            } // FIXME: no spec for version can be found regarding thesaurus
                        code.type == "BE-THESAURUS-PROCEDURES" ->
                            CDCONTENT().apply {
                        ...
                        code.type == "CD-VACCINEINDICATION" ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.CD_VACCINEINDICATION
                                sv = code.version
                                value =
                                    code.code
                            }
                        code.type?.startsWith("MS-EXTRADATA") ?: false ->
                            CDCONTENT().apply {
                                s = CDCONTENTschemes.LOCAL
                                sv = code.version
                                sl = code.type
     ...
     */
    private fun addHealthCareElement(
        trn: TransactionType,
        eds: HealthElement,
        itemIndex: Int = 0,
        config: Config,
            ...
            createItemWithContent(eds, itemIndex, itemType, content, "MF-ID")?.let {
                if (isHeANewVersionOf(eds) && config.format != Config.Format.PMF && eds.healthElementId != null) { // no versioning in PMF
                    it.lnks.add(
                        LnkType().apply {
                            type = CDLNKvalues.ISANEWVERSIONOF
                            url = makeLnkUrl(eds.healthElementId!!)
                        },
                    )
                }
     ...
     */
    private suspend fun getHealthElements(
        hcp: HealthcareParty,
        sfks: List<String>,
        config: Config,
    ): List<HealthElement> {
        val res =
            (
                healthElementLogic.listHealthElementsByHcPartyAndSecretPatientKeys(hcp.id, sfks).toList() +
                    (
     ...
     */
    private fun excludeHealthElementsForPMF(
        heList: List<HealthElement>,
        config: Config,
    ): List<HealthElement> =
    ...
    private suspend fun getAllContacts(
        ...
        val res =
            contactLogic.listContactsByHCPartyAndPatient(hcp.id, sfks.toList()).toList() +
                (
                    hcp.parentId?.let { parentId ->
     ...
     * @return a TransactionType.
     */
    suspend fun makeKinePrescriptionTransaction(
        contact: Contact,
        subCon: SubContact,
        form: Form,
        language: String,
     ...
     * @return a TransactionType.
     */
    suspend fun makeNursePrescriptionTransaction(
        contact: Contact,
        subCon: SubContact,
        form: Form,
        language: String,
     ...
     */
    private fun renderNursePrescriptionTemplate(
        contact: Contact,
        subCon: SubContact,
        language: String,
    ): ByteArray {
     ...
     */
    private fun getItemMFID(item: ItemType): String? = item.ids.find { it.s == IDKMEHRschemes.LOCAL && it.sl == "MF-ID" }?.value
     ...
     * @return the template as ByteArray
     */
    private fun renderKinePrescriptionTemplate(
        contact: Contact,
        subCon: SubContact,
        language: String,
    ): ByteArray {
    ...
    private fun renumberKmehrIds(folder: FolderType) {
        folder.transactions.fold(1) { transactionIndex, it ->
            val newTransactionIndex =
                it.ids.find { it.s == IDKMEHRschemes.ID_KMEHR }?.let { id ->
